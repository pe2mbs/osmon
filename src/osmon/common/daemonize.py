...
class Daemonize(object):
    ...
    def update_pid_file( self ):
        try:
            self._pid_lock_file.seek( 0, 0 )
            self._pid_lock_file.truncate(0)
            self._pid_lock_file.write( "%s" % (os.getpid()) )
            self._pid_lock_file.flush()
    ...
    def start(self):
        ...
        try:
            self._pid_lock_file = open(self.pid, "w")
        ...
        try:
            ...
            # locked.
            fcntl.flock( self._pid_lock_file, fcntl.LOCK_EX | fcntl.LOCK_NB )
        ...
        if not self.foreground:
            ...
            self.keep_fds.append( self._pid_lock_file.fileno() )
